...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .number-btn:hover { background: #5a5a5a; }
        .number-btn:active { background: #6a6a6a; }
        ...
        @media (max-width: 700px) {
            ...
            /* Detail grid bleibt 2-spaltig auf Mobile */
            .detail-grid { 
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="number-buttons">
            <button class="number-btn" onclick="sendShortcut('1')">1</button>
            <button class="number-btn" onclick="sendShortcut('2')">2</button>
            <button class="number-btn" onclick="sendShortcut('3')">3</button>
        </div>
            ...
            <div class="modal-body">
                <form id="editForm" method="post" action="/edit_album">
                    ...
                    <div class="detail-grid">
                        ...
                        <input type="text" name="albumartist" id="editArtist" style="background:#0c0c0c;color:#e0e0e0;border:1px solid #444;padding:8px;border-radius:4px;width:100%;">
                        
                        <label class="detail-label">Album:</label>
                        <input type="text" name="album" id="editAlbumName" style="background:#0c0c0c;color:#e0e0e0;border:1px solid #444;padding:8px;border-radius:4px;width:100%;">
                        
                        <label class="detail-label">Jahr:</label>
                        <input type="text" name="year" id="editYear" style="background:#0c0c0c;color:#e0e0e0;border:1px solid #444;padding:8px;border-radius:4px;width:100%;">
                        
                        <label class="detail-label">Genre:</label>
                        <input type="text" name="genre" id="editGenre" style="background:#0c0c0c;color:#e0e0e0;border:1px solid #444;padding:8px;border-radius:4px;width:100%;">
                    </div>
...
def edit_album():
    """Bearbeitet Album-Metadaten - FIXED"""
    ...
    try:
        ...
        # Baut modify command korrekt auf
        cmd = ["beet", "modify", "-y", f"id:{album_id}"]
        
        # Füge nur nicht-leere Felder hinzu
        if albumartist:
            cmd.append(f"albumartist={albumartist}")
        if album:
            cmd.append(f"album={album}")
        if year:
            cmd.append(f"year={year}")
        if genre:
            cmd.append(f"genre={genre}")
        ...
        if result.returncode != 0:
            print(f"Error modifying album: {result.stderr}")
        else:
            print(f"Successfully modified album: {result.stdout}")
